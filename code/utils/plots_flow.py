...
def plot(model, indices, plot_data, path, epoch, img_res, t, save_mesh=True,
         plot_nimgs=None, resolution=100, grid_boundary=3.0, level=0, t_idx=0, save_html=False):

    if plot_data is not None:
        cam_loc, cam_dir = rend_util.get_camera_for_plot(plot_data['pose'])

        plot_images(plot_data['rgb_eval'], plot_data['rgb_gt'], path, epoch, plot_nimgs, img_res,t_idx=t_idx)
        # plot normal maps
        plot_normal_maps(plot_data['normal_map'], path, epoch, plot_nimgs, img_res)
        if 'rgb_grid' in plot_data:
            plot_images(plot_data['rgb_grid'], plot_data['rgb_gt'], path, epoch, plot_nimgs, img_res, prefix='image_based_', t_idx=t_idx)
        if 'ang_flow' in plot_data:
            ang_flow = plot_data['ang_flow']
            plot_img(ang_flow,path,epoch,plot_nimgs,img_res,prefix='ang',t_idx=t_idx)

            t_flow = plot_data['t_flow']
            plot_img(t_flow,path,epoch,plot_nimgs,img_res,prefix='trans',t_idx=t_idx)
    if save_mesh:
        # data = []
        # plot surface
        try:
            ...
            implicit_network = model.module.implicit_network
            step = model.module.step
        surface_traces = get_surface_trace(path=path,
                                           epoch=epoch,
                                           sdf=lambda x, t: implicit_network(x, t, step=step, ord=0),
        ...
        if surface_traces is not None and save_html:
            for fn, st in enumerate(surface_traces):
                data = [st]
                ...
                filename = '{0}/surface_{1}_{2}.html'.format(path, epoch, fn)
...
def get_surface_trace(path, epoch, sdf, t=1, resolution=100, grid_boundary=[-2.0, 2.0], return_mesh=False, level=0):
    grid = get_grid_uniform(resolution, grid_boundary)
    points = grid['grid_points']

    z = []
    for pnts in tqdm(torch.split(points, 100000, dim=0)):
        z.append(sdf(pnts,t.unsqueeze(0))[0][:,:,:1].detach().cpu().numpy())
    z = np.concatenate(z, axis=0)
    z = z.astype(np.float32)
    meshexport_all = []
    traces = []
    for i in range(0, z.shape[1]):
        if (not (np.min(z[:,i]) > level or np.max(z[:,i]) < level)):
            ...
            traces.append(go.Mesh3d(x=verts[:, 0], y=verts[:, 1], z=verts[:, 2],
            ...
            meshexport.export('{0}/surface_{1}_{2}.ply'.format(path, epoch, i), 'ply')
            meshexport_all.append(meshexport)
    if return_mesh:
        return meshexport_all
    return traces
    # return None

def get_surface_high_res_mesh(sdf, t=1, resolution=100, grid_boundary=[-2.0, 2.0], level=0, take_components=True,
                              export_idx=[0,46]):
    # get low res mesh to sample point cloud
    grid = get_grid_uniform(100, grid_boundary)
    z = []
    points = grid['grid_points']
    for pnts in tqdm(torch.split(points, 10000, dim=0)):
        z.append(sdf(pnts,t.unsqueeze(0))[0][:,:,:1].detach().cpu().numpy())
    z = np.concatenate(z, axis=0)
    z = z.astype(np.float32)

    meshexport = []
    # for zi in range(0,z.shape[1],2):
    for zi in export_idx:
        z_tmp = z[:,zi]
        verts, faces, normals, values = measure.marching_cubes(
            volume=z_tmp.reshape(grid['xyz'][1].shape[0], grid['xyz'][0].shape[0],
                             grid['xyz'][2].shape[0]).transpose([1, 0, 2]),
            level=level,
            spacing=(grid['xyz'][0][2] - grid['xyz'][0][1],
                     grid['xyz'][0][2] - grid['xyz'][0][1],
        ...
        vecs = (torch.eig(s_cov.float(),eigenvectors=True)[1].transpose(0, 1))
        if torch.det(vecs) < 0:
        ...
        grid_aligned = get_grid(helper.cpu().float(), resolution)
        ...
        z_tmp = []
        for pnts in tqdm(torch.split(points, 10000, dim=0)):
            z_tmp.append(sdf(pnts,t.unsqueeze(0)[:,:zi+1])[0][:,:,:1].detach().cpu().numpy())
        z_tmp = np.concatenate(z_tmp, axis=0)
        z_tmp = z_tmp[:,-1]
        z_tmp = z_tmp.astype(np.float32)

        # meshexport = None
        if (not (np.min(z_tmp) > level or np.max(z_tmp) < level)):

            z_tmp = z_tmp.astype(np.float32)

            verts, faces, normals, values = measure.marching_cubes(
                volume=z_tmp.reshape(grid_aligned['xyz'][1].shape[0], grid_aligned['xyz'][0].shape[0],
            ...
            meshexport.append(trimesh.Trimesh(verts, faces, normals))
        else:
            meshexport.append([])
...
def get_surface_by_grid(grid_params, sdf, resolution=100, level=0, higher_res=False):
    ...
    if higher_res:
        ...
        s_cov = torch.mm(s_cov.transpose(0, 1), s_cov)
        # vecs = torch.view_as_real(torch.linalg.eig(s_cov)[1].transpose(0, 1))[:, :, 0]
        vecs = (torch.eig(s_cov,eigenvectors=True)[1].transpose(0, 1))
        if torch.det(vecs) < 0:
            vecs = torch.mm(torch.tensor([[1, 0, 0], [0, 0, 1], [0, 1, 0]]).cuda().float(), vecs)
        helper = torch.bmm(vecs.unsqueeze(0).repeat(recon_pc.shape[0], 1, 1),
                           (recon_pc - s_mean).unsqueeze(-1)).squeeze()
...
def get_grid_uniform_notcube(voxel_size = 0.02, xmin=np.array([-2.0, -2.0,-2.0]),xmax=np.array([2.0,2.0,2.0])):
    grid_size = (xmax-xmin)//voxel_size
    x = np.arange(grid_size[0])*voxel_size + xmin[0]
    y = np.arange(grid_size[1])*voxel_size + xmin[1]
    z = np.arange(grid_size[2])*voxel_size + xmin[2]

    xx, yy, zz = np.meshgrid(x, y, z)
    grid_points = torch.tensor(np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=torch.float)

    return {"grid_points": grid_points.cuda(),
            "shortest_axis_length": np.min(xmax-xmin),
            "xyz": [x, y, z],
            "shortest_axis_index": np.argmin(xmax-xmin)}
...
def plot_normal_maps(normal_maps, path, epoch, plot_nrow, img_res,t_idx=0):
    cuda_idx = normal_maps.device.index
    ...
    img.save(f'{path}/normal_{epoch}_{cuda_idx}_{t_idx}.png')


def plot_img(img, path, epoch, plot_nrow, img_res, prefix,t_idx=0):
    cuda_idx = img.device.index
    normal_maps_plot = lin2img(img, img_res)
    ...
    img = Image.fromarray(tensor)
    img.save(f'{path}/{prefix}_{epoch}_{cuda_idx}_{t_idx}.png')

def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res, prefix='', t_idx=0):
    ground_true = ground_true.cuda()
    cuda_idx = rgb_points.device.index
    ...
    img = Image.fromarray(tensor)
    img.save(f'{path}/{prefix}rendering_{epoch}_{cuda_idx}_{t_idx:03d}.png')
