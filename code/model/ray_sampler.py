...
class UniformSampler(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, take_sphere_intersection=False, far=-1, **kargs):
...
class ErrorBoundSampler_WTime(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, N_samples_eval, N_samples_extra,
                 eps, beta_iters, max_total_iters,
                 inverse_sphere_bg=False, N_samples_inverse_sphere=0, add_tiny=0.0):
        super().__init__(near, 2.0 * scene_bounding_sphere)
    ...
    def get_z_vals(self, ray_dirs, cam_loc, T, model):
        """
        T: [fn]
        """
        beta0 = model.density.get_beta().detach()
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            # Calculating the SDF only for the new sampled points
            with torch.no_grad():
                samples_sdf = model.implicit_network.get_sdf_vals(points_flat, T.unsqueeze(0), step=model.step, ord=0)
            if samples_idx is not None:
                sdf_merge = torch.cat([sdf.reshape(-1, z_vals.shape[1] - samples.shape[1]),
                                       samples_sdf.reshape(-1, samples.shape[1])], -1)
                sdf = torch.gather(sdf_merge, 1, samples_idx).reshape(-1, 1)
            ...
            first_cond = a.pow(2) + b.pow(2) <= c.pow(2)
            second_cond = a.pow(2) + c.pow(2) <= b.pow(2)
            d_star = torch.zeros(z_vals.shape[0], z_vals.shape[1] - 1).cuda()#.to(device=b.device,dtype=b.dtype)
            d_star[first_cond] = b[first_cond]
            d_star[second_cond] = c[second_cond]
            s = (a + b + c) / 2.0
            area_before_sqrt = s * (s - a) * (s - b) * (s - c)
            mask = ~first_cond & ~second_cond & (b + c - a > 0)
            # d_star = d_star.to(dtype=area_before_sqrt.dtype)
            d_star[mask] = (2.0 * torch.sqrt(area_before_sqrt[mask])) / (a[mask])
            d_star = (d[:, 1:].sign() * d[:, :-1].sign() == 1) * d_star  # Fixing the sign
    ...
    def get_z_vals_simpler(self, ray_dirs, cam_loc, T, model):
        """
        T: [fn]
        """
        N = 128
        iter = 4
        beta0 = model.density.get_beta().detach()
        beta = torch.ones_like(beta0)*self.scene_bounding_sphere*4/N
        decay = (beta0/beta).pow(1/(iter-1))
        ...
        for i in range(iter):
            ...
            points_flat = points.reshape(-1, 3)
            with torch.no_grad():
                sdf_new = model.implicit_network.get_sdf_vals(points_flat, T.unsqueeze(0), step=model.step, ord=0)
            if samples_idx is not None:
                sdf = torch.gather(torch.cat([sdf,sdf_new.reshape([-1,N])],dim=-1),dim=1,index=samples_idx)
            else:
                sdf = sdf_new
            sdf = sdf.reshape(z_vals.shape)
            # Upsample more points
            dists = z_vals[:, 1:] - z_vals[:, :-1]
            density = model.density(sdf, beta=beta.unsqueeze(-1))

            dists = torch.cat([dists, torch.tensor([0.02]).cuda().unsqueeze(0).repeat(dists.shape[0], 1)], -1)
            free_energy = dists * density
            shifted_free_energy = torch.cat([torch.zeros(dists.shape[0], 1).cuda(), free_energy[:, :-1]], dim=-1)
            alpha = 1 - torch.exp(-free_energy)
            transmittance = torch.exp(-torch.cumsum(shifted_free_energy, dim=-1))
            ...
            if i == (iter-1):
                samples = self.sample_from_cdf((z_vals[:,1:]+z_vals[:,:-1])/2, weights[:,1:], self.N_samples)
            else:
                samples = self.sample_from_cdf((z_vals[:,1:]+z_vals[:,:-1])/2, weights[:,1:], N)
                z_vals, samples_idx = torch.sort(torch.cat([z_vals, samples], -1), -1)
                beta = beta*decay
    ...
    def sample_from_cdf(self, bins, weights, n_sample):
        # bins = z_vals
        ...
        # Invert CDF
        u = torch.rand(list(cdf.shape[:-1]) + [n_sample]).cuda()
        u = u.contiguous()
...
class HierarchicalSampler_WTime(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, N_samples_eval, N_samples_extra,
                 eps, beta_iters, max_total_iters,
                 inverse_sphere_bg=False, N_samples_inverse_sphere=0, add_tiny=0.0):
        super().__init__(near, 2.0 * scene_bounding_sphere)
    ...
    def get_z_vals(self, ray_dirs, cam_loc, T, model):
        ...
        iter = 4
        beta0 = model.density.get_beta().detach()
        beta = torch.maximum(beta0, torch.ones_like(beta0)*self.scene_bounding_sphere*4/N)
        decay = (beta0/beta).pow(1/(iter-1))
